...
let basic_tests ( ctx : context ) =
  ...
  let add = (mk_add ctx [x; mk_numeral_int ctx 1 (Integer.mk_sort ctx)]) in
  let eq = (mk_eq ctx y add) in
  ...
  (
    ...
    if (check solver []) != SATISFIABLE then
      ...
      match model with
        ...
        | Some m -> let x_value = Model.get_const_interp_e m x in
          Printf.printf "x value - %s\n" (Integer.numeral_to_string (Option.get x_value));
...
**)
let  _quantifier_example1 ( ctx : context ) =
